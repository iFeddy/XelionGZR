using System;
using System.IO;
using System.Net;
using System.ServiceProcess;

namespace XelionZR
{
    public partial class Main : ServiceBase
    {
        public static string responseurl = "http://127.0.0.1:58492/";
        public static string responsestring = "[1440][3][{( USA V.0.001 )}]\\n4903026713C2FDA\n";
        public bool isRunning = false;
        public zResponder ws;

        public static string SendResponse(HttpListenerRequest request) =>
            responsestring;

        public Main()
        {
            base.ServiceName = "_XelionZR";
            InitializeComponent();
        }

        protected override void OnStart(string[] args)
        {
            if (this.isRunning)
            {
                base.Stop();
            }
            else
            {
                string[] prefixes = new string[] { responseurl };
                this.ws = new zResponder(new Func<HttpListenerRequest, string>(Main.SendResponse), prefixes);
                try
                {
                    this.ws.Run(responseurl);
                    this.isRunning = true;
                }
                catch (Exception exception)
                {
                    using (StreamWriter writer = new StreamWriter("./ExceptionError.txt", true))
                    {
                        writer.Write(string.Format("Message: {0}<br />{1}StackTrace :{2}{1}Date :{3}{1}-----------------------------------------------------------------------------{1}", new object[] { exception.Message, Environment.NewLine, exception.StackTrace, DateTime.Now.ToString() }));
                    }
                }
            }
        }
        protected override void OnStop()
        {

        }

    }
}